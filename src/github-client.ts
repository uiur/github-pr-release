...
class GithubClient {
  ...
  assignReviewers  (pr, prs) {
    ...
    return this.post(this.pullRequestEndpoint() + '/' + pr.number + '/requested_reviewers', { reviewers }).then(function (res: any) {
      return res.body
    })
  }

  updatePR  (pr, data) {
    return this.patch(this.pullRequestEndpoint() + '/' + pr.number, data).then(function (res: any) {
      return res.body
    })
  }
}
